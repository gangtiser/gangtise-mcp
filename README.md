...
- **0.2.6 — 2026-09-06**：同步 CLI v0.38.0。🔴 修四处静默错数：`valuation_analysis` 的 `skipNull`+`fieldList` 会把正常数据全过滤成 0 行、全市场分片按位置合并可致开收盘价互换、正文续读误判格式会丢掉后半段、重复列名静默少一列。客户端取消后不再继续发分页/分片请求。行情 `fieldList` 缺列改为标 `missingFields`，且**只返回点名的列、不自动附带身份列**。新增沪深 ETF 与 20 个全球指数；`realtime` 新增 `tradeStatus`、**不再返回 `turnoverRate`/`volumeRatio`**；`stock_summary` 上限收到 5000。
