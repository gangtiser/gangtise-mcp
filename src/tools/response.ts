...
export const responseFamily: FamilyModule = {
  name: "response",
  endpoints: {},
  tools: [
    defineTool({
      ...
      run: async (_ctx, args) => {
        const { saved_to, offset = 0, limit = DEFAULT_LIMIT, fields } = args as { saved_to: string; offset?: number; limit?: number; fields?: string[] }
