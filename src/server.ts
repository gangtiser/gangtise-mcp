 ...
 * 描述也不该承担跨工具路由。
 * 预算：dateContextInstruction() 168B + 本常量 1,583B = 1,751B ≤ 1,800B。
 * 改动前先量字节，别手推。
 */
...
①行情/财务：日K与三表按市场用 _hk/_us；realtime 单工具覆盖三市场；分钟K/指数K/资金流仅 A 股。行情/估值/财务/盈利预测/股东/主营优先专用工具；证券长尾指标 indicator_*(EDE)，宏观行业 edb_*。
...
计费见各工具【积分】标签，未标注即免费；优先免费或低价专用工具，慎用全市场/超大 size/fetchAll。`
