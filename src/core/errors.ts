...
const ERROR_HINTS: Record<string, string> = {
  ...
  // 此码现在盖三层：list 类的单页上限（50）、全市场 K 线的行数上限，以及 EDE 截面 /
  // 时序的单次单元格上限。两个 indicator 工具没有 size 可调，只能按乘积拆批，所以
  // 单独给出口径——否则「调小 size」对它们是一句无法执行的建议。
  "100006": "缩短日期范围或调小 size / limit（list 类接口单页上限为 50）；EDE 指标截面 / 时序另有单次单元格上限（截面 = 证券数 × 指标数，时序 = 序列数 × 日期数，msg 里写明了限额），超出请按证券或指标分批、或缩短日期区间。注意 securityCodeList 里传板块 ID 时服务端会展开成全部成分股，实际证券数远大于写进参数的条数。",
  ...
  // 两句都被探针钉住：单个日期也会报此码（EDE 条件选股传一个较早的 date），
  // 此时根本没有窗口可缩，所以不写「请缩小日期范围」；而这条边界是按账号配的，
  // 三个 EDE 端点与 quote 日 K 逐日同界，所以也不写「改用范围更宽的同族工具」——
  // 那会让调用方多花一次注定同样失败的请求。归因到账号与 server.instructions 的
  // 「取数窗口随账号权限变化」一致。
  "110003": "查询时间超出本账号的数据权限范围——把日期改到更近的范围内；整段区间都在界外时缩短窗口没有用（单个日期同样会报此码）。这个范围按账号配、不按接口配，换同族的另一个工具同样报此码；要更长历史需联系客户经理开通。",
  ...
  // 所以提示保留对 fileType 的指引。云盘对不存在（含已被删除）的文件 / 文件夹 ID 也回此码。
  "130002": "确认 ID 有效且本账号可见（云盘的文件 / 文件夹已被删除也回此码）；下载类还需检查 fileType 取值是否合法。",
  ...
  "230003": "股票池数量已达账号上限（30 个）——先用 gangtise_stock_pool_delete 删掉不用的池再建。",
  "230006": "池名与该账号已有的池重复——先用 gangtise_stock_pool_list 看现有名称，换一个再建 / 改名。",
  "230007": "池名超过 10 个字符（按字符计，中文算 1 个）——缩短后重试。",
  // 云盘管理的三个专有码。名称按 UTF-16 计，emoji 算 2。
  "230004": "云盘文件 / 文件夹名称超过 200 个字符（按 UTF-16 计，中文算 1、emoji 算 2）——缩短后重试。",
  "230005": "文件夹不能移到自身、自己的子文件夹或另一个空间——换一个同空间、不在其下级的目标文件夹。",
  "230008": "试用账号当日累计上传已超过 500MB——次日再传，或联系客户经理。",
  "240001": "换更早的报告期。",
  "240002": "改述后重新提交。",
  "240003": "对照工具参数说明检查取值。",
