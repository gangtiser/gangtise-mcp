...
- **配额会长期低估实际占用，超额一直留在盘上**。目录大小的缓存有三条路会失准：把一次还在写盘的下载的中间大小当成最终值；两次扫描交错时旧的一次把新读数覆盖回去；目录释放后登记信息不清、让在途的扫描把已删目录重新写回缓存。现在只缓存写入方已声明写完的目录，测量前后都要确认这期间没有再次写完，摘除走同一个入口、四份登记同生共死。
