...
现在支持 A股/港股/美股个股 + 交易所指数（沪深京）+ 概念指数（`.GT`）+ 申万行业指数（`.SWI`），**可在一次请求里混着传**。⚠️ **中信行业指数 `.CI` 不在其中**——统一日 K 线与实时快照都对 `.CI` 回「证券代码无效」，而 `gangtise_securities_search` 会把 `.CI` 码当作合法指数返回，所以描述里明确写了这条边界并指向 `gangtise_index_day_kline`。因此本工具原有的「A 股专用」后缀校验已移除——它会拒掉现在完全合法的港股/美股/指数查询。
